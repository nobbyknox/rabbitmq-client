...
class RabbitMQClient extends EventEmitter {
    ...
    createReceiveChannel(next) {
        ...
        clazz.amqpConn.createChannel(function (err, ch) {
            ...
            console.log('receiveChannel created');
            next();
        });
    }
    ...
    subscribe(queueName, callback) {
        ...
        clazz.receiveChannel.assertQueue(queueName, { durable: true }, (err) => {
            ...
            clazz.receiveChannel.consume(queueName, (msg) => { clazz.messagePreHandler(callback, msg, clazz.receiveChannel); }, options);
            console.log(`Consumer on queue "${queueName}" registered`);
        });
    }
    ...
    publish(queueName, message, callback) {
        ...
        if (clazz.pubChannel) {
            try {
                clazz.pubChannel.publish('', queueName, message, { persistent: true }, (err, ok) => {
                    if (err) {
    ...
    messagePreHandler(callback, msg, channel) {
        callback(msg, (err) => {
            if (err) {
